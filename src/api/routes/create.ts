...
export async function createRoutes(app: FastifyInstance): Promise<void> {
  app.post('/api/v1/create-orders', async (req, reply) => {
    ...
    const log = req.log.child({ clint_deal_id: body.clintDealId, pipeline: body.pipeline });

    // a. Idempotência
    const jaCriado = await checkIdempotency(body.clintDealId);
    if (jaCriado) {
      log.info('ignored_duplicate');
      const resp: CreateOrdersResponse = {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        break; // para no primeiro erro pra não estourar o rate limit em cima de falha
      }
