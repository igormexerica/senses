/**
 * Camada de dados do Comparativo — SERVER ONLY.
 *
 * Acesso direto ao Postgres do Supabase self-hosted via `DATABASE_URL`. A string
 * de conexão NUNCA vai pro client: este módulo importa "server-only", então
 * qualquer import acidental no client quebra o build.
 *
 * Chama as funções SQL de data/schema.sql (não recriadas aqui):
...
export async function ultimaAtualizacao(): Promise<string | null> {
  const { rows } = await pool().query("select max(atualizado_em) as m from analytics.fato_diario");
  const m = rows[0]?.m;
...
/** Carrega os 3 períodos + deltas (P2/P1, P3/P2, P3/P1) + última atualização. */
