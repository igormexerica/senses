 ...
 * Lista sempre do mais antigo (P1) ao mais recente (P3) — sustenta a escala
 * de cor "claro→escuro = antigo→agora".
 */
...
/** Presets selecionáveis. Sliding (7/14/30d) geram 3 janelas do MESMO tamanho. */
...
/** 3 janelas para um preset, ancoradas em `hoje` (YYYY-MM-DD). */
export function periodosDoPreset(preset: Exclude<Preset, "custom">, hoje: string): Periodo[] {
  if (preset === "mtd") {
    // Mesma janela "dia 1 → dia N" em 3 meses. Mês mais curto que N (ex.: fev × dia
    // 31) corta no último dia do mês — janelas nunca se sobrepõem nem invadem o mês
    // seguinte (evita dupla contagem). Por isso o tamanho pode variar 1–3 dias.
 ...
 * - Com os 6 p{1..3}{ini,fim} presentes → custom (valida; pode lançar).
